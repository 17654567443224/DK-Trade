...
from crawl_data import creat_table, download_future, conn_database, conn_strategy_database, create_strategy_position_table, create_strategy_orders_table
...
class SimulationEngine(threading.Thread):
    def __init__(
            ...
            lever,
            symbol_selection,
            open_position,  # 开仓策略实例 (需实现 check_entry(symbol, data) -> bool)
            profit_loss,  # 止盈止损策略实例 (需实现 calculate_exit(symbol, entry_price, data) -> (tp, sl))
            fund,  # 资金策略实例 (需实现 calculate_position(symbol, price, tp, sl) -> sz)
            ws: WebSocketProducer,  # 实时数据生产者
            interval: Interval,
            max_positions: int,
            account,   # 初始账户信息
            funSymbol_selection,
        ...
        super().__init__()
        self.id = id
        self.symbol_selection = symbol_selection
        ...
        # 数据订阅
        if hasattr(self.ws, 'register_consumer'):
            print(5555)
        self.data_queue = self.ws.register_consumer()
        self.running = False
        self.init_balance = float(self.account["balance"])
        self.bars = {}
        self.symbol_ls = []
        self.ss_fun = self.funSymbol_selection['func']
        self.op_fun = self.funOpen_position['func']
        self.pl_fun = self.funProfit_loss['func']
        self.fd_fun = self.funFund['func']
        self.ss_method = getattr(self.symbol_selection, self.ss_fun)
        self.op_method = getattr(self.open_position, self.op_fun)
        self.pl_method = getattr(self.profit_loss, self.pl_fun)
        self.fd_method = getattr(self.fund, self.fd_fun)
                ...
                if 'event' in res:
                    ...
                    if "candle" in mode:
                        ...
                        if symbol in self.account["position"]:
                            ...
                            self._check_exit_condition(symbol, price)
            except Empty:
                continue  # 无新数据时跳过
    ...
    def _process_data(self, am: ArrayManager, symbol):
        """处理单条市场数据"""
        # # 1. 更新选股池
        # if isinstance(self.symbols_select, str):
        #     symbols = self.symbols_select
        # else:
        #     symbols = getattr(self.symbols_select, self.fun_dict["symbols_select"])
        # 2. 遍历候选标的执行策略
        # 3. 检查是否已有持仓
    ...
    def _execute_order(self, symbol: str, td:int):
        ...
        tp_price = self.match_precision(price, tp_price)
        sl_price = self.match_precision(price, sl_price)
        ...
        if td == 0:
            ...
            if sl_price < sz[1]:
                sl_price = sz[1]
        # price, sz = self.match_order(price, volume=sz[0], order_type=order_type, asks=asks, bids=bids)
    ...
    def _check_exit_condition(self, symbol: str, price):
        """检查止盈止损是否触发"""
    ...
    def _close_position(self, symbol: str, exit_price: float, exit_type: str):
        ...
        if posSide == "long":
            # close, vol = self.match_order(Decimal(exit_price), Decimal(sz), order_type="sell", asks=asks, bids=bids)
            #
            # # 滑点调整（可选）
            ...
            # close, vol = self.match_order(Decimal(exit_price), Decimal(sz), order_type="buy", asks=asks, bids=bids)
            #
            # # 滑点调整（可选）
    ...
    def match_order(self, price, volume, order_type: str, asks: List[List[Union[str, float]]],
        ...
        if order_type == "buy":
            ...
            try:
                ...
                if valid_asks:
                    closest_ask = min(valid_asks, key=lambda x: Decimal(x[0]))
