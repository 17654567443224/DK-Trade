...
class MACDStrategy(Base_Template):
    """
    MACD（移动平均线趋同散度）交易策略
    
    策略说明：
    - MACD金叉（DIFF上穿DEA）时，产生做多信号
    - MACD死叉（DIFF下穿DEA）时，产生做空信号
    - 使用MACD柱状图的大小来判断趋势强度
    ...
    def default_profit_loss(self, symbol: str, td: int, price: float) -> Tuple[float, float]:
        """
        默认止盈止损策略：基于MACD柱状图的动态止盈止损
        """
        try:
            # 计算MACD指标
            dif, dea, macd = self.bars[symbol].macd(
                self.fast_period,
                self.slow_period,
                self.signal_period,
                array=True
            ...
            # 使用MACD柱状图的大小来调整止盈止损幅度
            macd_height = abs(macd[-1])
            base_range = price * 0.01  # 基础1%
            
            # 根据MACD柱状图高度调整止盈止损范围
            adjusted_range = base_range * (1 + macd_height)
            
            if td == 1:  # 做多
                tp_price = price * (1 + adjusted_range * 2)  # 止盈为止损的2倍
                sl_price = price * (1 - adjusted_range)
            else:  # 做空
                tp_price = price * (1 - adjusted_range * 2)
                sl_price = price * (1 + adjusted_range)
    ...
    def default_fund(self, td: int, symbol: str, price: float, lever: float = None) -> Tuple[float, float]:
        """
        默认资金管理策略：基于MACD趋势强度的动态仓位，考虑杠杆因素
        """
        try:
            # 计算MACD指标
            dif, dea, macd = self.bars[symbol].macd(
            ...
            # 计算趋势强度
            trend_strength = abs(dif[-1] - dea[-1]) / price
            
            # 根据趋势强度调整仓位（最大使用账户余额的20%）
            base_position = self.account["balance"] * Decimal('0.2')
            trend_factor = min(trend_strength * 100, 1)  # 将强度转换为0-1之间
            position_size = base_position * Decimal(str(trend_factor))
    ...
    def _close_position(self, symbol, price, reason=""):
        ...
        """
        try:
            position = self.account["position"].get(symbol)
            if not position:
                return
                
            # 记录交易
            self.account["orders"][f"{symbol}_{datetime.now().timestamp()}"] = {
                "symbol": symbol,
                "type": "close",
                "side": "sell" if position["posSide"] == "long" else "buy",
                "price": price,
                "size": float(position["sz"]),
                "pnl": float(position["pnl"]),
                "reason": reason
            }
            
            # 更新账户余额
            self.account["balance"] += float(position["pnl"])
            
            # 移除持仓
            del self.account["position"][symbol]
