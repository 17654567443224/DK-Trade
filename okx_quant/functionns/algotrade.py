...
class websocket_funs:
                ...
                for i in result:
                    if i['maxSz'] is None or i['minSz'] is None:
                        ...
                        if Decimal(size) <= Decimal(i['maxSz']) and Decimal(size) >= Decimal(i['minSz']):
                            imr = i['imr']
                            mmr = i['mmr']
                            liq_price = self.calculate_liquidation_price(imr, ctVal, size, price, mmr, setting.order_fee, tradingview)
    ...
    def trade_logic(self, trading_view, bv, lever, symbol, price, ba, ctVal, avg_val, algo):  # trade_logic  tradingview, bv, lever, symbol, price, ba, ctVal
        ...
        los_loss = avg_val / Decimal(self.stop_loss)
        los_perfit = avg_val / Decimal(self.profit_per)
        ...
        if algo != trading_view and self.multiplier != 1:
            if val > 100 / lever:
                self.trade_switch = True
                return
            if 0.75 > los_loss > 0.3:
                stop_loss = Decimal(self.stop_loss) * los_loss

        elif algo == trading_view and self.multiplier != setting.max_fail_count:
            if los_perfit > 0.75:
                stop_perfit = Decimal(self.profit_per) * los_perfit
    ...
    def oco(self, symbol, tradingview, avgpx, liqpx, lever, sz, stop_loss, stop_perfit):
        ...
        result = self.tradeAPI.place_algo_order(symbol, 'isolated', side, ordType='oco', tag=self.close_tag,
                                                ...
                                                slTriggerPx=str(sl_tg), slOrdPx=str(slPx),
