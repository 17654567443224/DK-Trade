...
class websocket_funs:
    ...
    def price_monitor(self, result, symbol, data):
        if symbol in result:
            if (Decimal(data[0][2]) - Decimal(result[symbol][0])) / Decimal(result[symbol][0]) > (Decimal(self.quick_tradingview) / Decimal('2')) and Decimal(data[0][3]) > Decimal(result[symbol][1]):
                try:
                    res = self.marketAPI.get_markprice_candlesticks(instId=symbol, limit="21",
                                                                    bar=setting.rest_interval)
                    code = res.get('code')
                    if code == '0':
                        new_res = deal_message(res)
                    else:
                        print(code)
                        return
                except:
                ...
                for kline in res_:
                    ...
                    change = ((high_price - low_price) / low_price) * Decimal('100')
                    max_change = max(max_change, abs(change))
                new_change = (Decimal(new_res[0][2]) - Decimal(new_res[0][3])) / Decimal(new_res[0][3])
                new_change = new_change * Decimal('100')
                if new_change / max_change > Decimal('1.05') and new_change / max_change < Decimal('2'):
                    new_change = new_change.quantize(Decimal('0.00'))
                    self.wx.price_monitor(symbol, new_change, "建仓", "一级")
                elif new_change / max_change > Decimal('2') and new_change / max_change < Decimal('3'):
                    new_change = new_change.quantize(Decimal('0.00'))
                    self.wx.price_monitor(symbol, new_change, "建仓", "二级")
                elif new_change / max_change > Decimal('3'):
                    new_change = new_change.quantize(Decimal('0.00'))
                    self.wx.price_monitor(symbol, new_change, "建仓", "三级")
            elif (Decimal(result[symbol][1]) - Decimal(data[0][3])) / Decimal(result[symbol][1]) > (Decimal(self.quick_tradingview) / Decimal('2')) and Decimal(data[0][2]) < Decimal(result[symbol][0]):
                try:
                    res = self.marketAPI.get_markprice_candlesticks(instId=symbol, limit="21",
                                                                    bar=setting.rest_interval)
                    code = res.get('code')
                ...
                if new_change / max_change > Decimal('1.05') and new_change / max_change < Decimal('2'):
                    new_change = new_change.quantize(Decimal('0.00'))
                    self.wx.price_monitor(symbol, new_change, "砸盘", "一级")
                elif new_change / max_change > Decimal('2') and new_change / max_change < Decimal('3'):
                    new_change = new_change.quantize(Decimal('0.00'))
                    self.wx.price_monitor(symbol, new_change, "砸盘", "二级")
                elif new_change / max_change > Decimal('3'):
                    new_change = new_change.quantize(Decimal('0.00'))
                    self.wx.price_monitor(symbol, new_change, "砸盘", "三级")
    ...
    def check_pending(self, symbol):
        ...
        return True
    @retry_on_exception_sync
    def trade_logic(self, trading_view, bv, lever, symbol, price, ba, ctVal, avg_val, algo):  # trade_logic  tradingview, bv, lever, symbol, price, ba, ctVal
        self.dynamic_param(symbol)
        pos_val = math.log(self.multiplier, setting.Incremental_multiplier)  # 27 3**3
        a = abs(avg_val * 100)
        val = int(math.log(a, 2))
        if algo != trading_view and self.multiplier != 1:
            ...
            if pos_val - val > 1:
                if pos_val > 2:
                    pos_val = 2
                self.multiplier = self.multiplier * 3 ** val
            
        if pos_val <= setting.add_extra:  # 2
            usdt = bv / Decimal(setting.total_fail_count)
        else:
            usdt = bv / Decimal(setting.total_fail_count) * Decimal(self.multiplier) / Decimal(setting.Incremental_multiplier) ** Decimal(setting.add_extra)
        ...
        if trading_view == 1:  # 多
            ...
            res = self.tradeAPI.place_order(instId=f'{symbol}-SWAP', tdMode='isolated', side='sell', tag=self.open_tag,
                                            posSide='short',
                                            ordType='limit', sz=sz, px=str(price))
            order_id = deal_message(res)[0].get('ordId')
            code = res.get('code')
            if code != '0':
                self.trade_switch = True
    ...
    def trading(self, tradingview, symbol, price, ts):  # 1.get_banlance  2.get_max_lever and position_cont(usdt)
        ...
        for i, data in enumerate(pre):
            if data['below_avg_changes']:
                avg = sum(data['below_avg_changes']) / len(data['below_avg_changes']) / 4**i
                avg_temp.append(avg)
        ...
        percent_val = (sum(percent_temp) / len(percent_temp))
        if percent_val > 0.5:
            percent_val = 0.5
        avg_val = (sum(avg_temp) / len(avg_temp)) * (0.5 + percent_val) * 0.75
        count_above_1 = sum(1 for i in percent_temp if i >= 1)
        if count_above_1 > len(percent_temp):  # 判断是否为 "long"
            algo = 1 if tradingview == 1 else 0
        else:
            algo = 0 if tradingview == 0 else 1
    ...
    def run_position(self, result):
        for i in result:
            ...
            if i['uplRatio'] and symbol in self.strike_sym_ls and self.multiplier < setting.max_fail_count:
                if Decimal(i['uplRatio']) >= Decimal(self.profit_per) * Decimal("1.05"):  # Profit
                    price = i['markPx']
                    self.close_position(symbol, mode, sz)

                elif Decimal(i['uplRatio']) > self.profit_min and Decimal(i['uplRatio']) < self.profit_min + self.float_val:  # trends_Stop Loss
                    ...
                    while True:
                        ...
                        if 'event' in res:
                            ...
                            if self.avg_switch and not self.trade_switch:
                                try:
                                    self.price_monitor(self.price_dict, symbol, data)
                                except:
                                    continue
                                continue
