        ...
        if result:
            for i in result:
                if i.get('state') == 'live':
                    ...
                    time.sleep(0.1)
                    if e_time > setting.order_wait_time and sym_swap in self.order_dict[sym] and i['algoId'] == "":
    ...
    def get_price(self, symbol: list, q: Queue):  # load_interval：根据多少跟kline计算
        ...
        for i in symbol:
            ...
            while True:  # 无限循环直到获取成功
                try:
                    result = deal_message(self.marketAPI.get_markprice_candlesticks(instId=i, limit=setting.load_interval, bar=setting.rest_interval))
                    break  # 成功获取，退出无限循环
                except:
            ...
            if (max_high_price - min_high_price) > Decimal(setting.quick_tradingview)*cal_price and (max_low_price - min_low_price) > Decimal(setting.quick_tradingview)*cal_price:
                max_high_price = result[0][2]
                min_low_price = result[0][3]
    ...
    def cal_tradingview(self, result, symbol, data):
        for r in result.keys():
            if symbol == r and symbol not in self.strike_sym_ls and symbol not in self.order_dict:
                if Decimal(data[0][2]) > Decimal(result[r][0]) and Decimal(data[0][3]) > Decimal(result[r][1]):  # 0：时间戳 1：o 2：h 3：l 4：c
                    ...
                    return
                elif Decimal(data[0][3]) < Decimal(result[r][1]) and Decimal(data[0][2]) > Decimal(result[r][0]):
                    trading_view = 0  # 空
                    print("触发开空")
            ...
            try:
                ...
                for i in result:
                    ...
                    if i['uplRatio']:
                        ...
                        elif Decimal(i['uplRatio']) > setting.profit_min and Decimal(i['uplRatio']) < setting.profit_min + setting.float_val:  # 动态止盈
                            if int(time.time()) - sym_time > setting.position_update and self.profit_switch == False:
                                price = i['markPx']
                                self.profit_switch = True
                                self.close_position(symbol, mode, price, sz)
                                sym_time = 99999999999

                            elif sym_time == 99999999999:
                                sym_time = int(int(i['uTime']) / 1000)
                ...
                async with websockets.connect(url) as ws:
                    ...
                    while True:
                        ...
                        if 'event' in res:
                            ...
                            if sever_time % setting.update_interval <= 2:  # 如果新的interval更新时间小于10秒，则认为新的kline出现，更新数据  ////////////////////////////////////////////////
                                print(sever_time)
                                self.avg_switch = False
                            ...
                            if g_thread.is_alive():
                                continue
                            # 判断有无开仓条件
                            if self.trade_switch and symbol not in self.ignore_temp:
