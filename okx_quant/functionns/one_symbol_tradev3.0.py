...
class websocket_funs:
    ...
    @retry_on_exception_decorator
    async def spot_trading(self, tradingview, symbol, price):  # 1.获取账户余额  2.获取交易对最大可开杠杆倍数和开仓数量
        bv = self.get_banlance_avail()
    ...
    def run_position(self, result):
        for i in result:
            ...
            if symbol in self.strike_sym_ls and symbol not in self.order_algo:  # self.strike_sym_ls????????
                if mode == 'long':
                    side = 'sell'
                    posSide = 'long'
                ...
                if i['uplRatio']:
                    if Decimal(i['uplRatio']) >= Decimal(self.profit_per):  # 止盈  没什么用
                        price = i['markPx']
                        ...
                        self.position_time = 0
            elif symbol in self.spot_order and symbol not in self.spot_profit_dict:
                if mode == 'long':
                    side = 'sell'
                    posSide = 'long'
                        ...
                        if 'event' in res:
                            ...
                            if isinstance(self.fluctuation_values, list) and self.trade_switch == True:
                                if sym_idx == len(self.fluctuation_values) - 1:
                                    ...
                                    sym_idx = sym_idx + 1
                                    continue
                                elif self.fluctuation_values[sym_idx] == symbol and symbol not in self.ignore_temp and symbol not in self.spot_order:
        ...
        for i in data:
            ...
            if i.get('state') == 'filled' and i['side'] == 'buy':  #  买单成交
                if symbol in self.spot_profit_dict:
                    ...
                    continue
                elif symbol not in self.strike_sym_ls:
