...
class websocket_funs:
        ...
        if symbol in result:
            if symbol not in self.strike_sym_ls and symbol not in self.order_dict:
                if Decimal(data[0][2]) > Decimal(result[symbol][0]) and Decimal(data[0][3]) > Decimal(result[symbol][1]):  # 0：timestamp 1：o 2：h 3：l 4：c
                    try:
                        res = self.marketAPI.get_markprice_candlesticks(instId=symbol, limit="3",
                                                                        bar=setting.rest_interval)
                        code = res.get('code')
                        if code == '0':
                            new_res = deal_message(res)
                    ...
                    if time_ < 240:
                        new_res = new_res[1:]
                    if (Decimal(new_res[0][2]) - Decimal(new_res[1][2])) / (Decimal(new_res[0][2]) - Decimal(new_res[0][3])) > 0.5:
                    ...
                    if time_ < 240:
                        new_res = new_res[1:]
                    if (Decimal(new_res[1][3]) - Decimal(new_res[0][3])) / (Decimal(new_res[0][2]) - Decimal(new_res[0][3])) > 0.5:
                        trading_view = 0  # short
                        print("Trigger_short", symbol)
                        price = Decimal(data[0][3]) * Decimal(1.0005)  # 1.0005
                        price = self.match_precision(Decimal(price), Decimal(data[0][3]))
    ...
    def run_position(self, result):
        for i in result:
            ...
            if symbol in self.strike_sym_ls and symbol not in self.order_algo and symbol not in self.spot_ls and symbol not in self.order_dict:  # self.strike_sym_ls????????
                if mode == 'long':
                    side = 'sell'
                    posSide = 'long'
            ...
            elif symbol not in self.strike_sym_ls and symbol not in self.order_algo:
                self.strike_sym_ls.append(symbol)
                print("error1", self.order_dict)

            elif symbol in self.strike_sym_ls and symbol in self.order_dict and symbol not in self.order_algo:
                del self.order_dict[symbol]
                print("error2")

            elif symbol in self.strike_sym_ls and self.trade_switch == True:
                self.strike_sym_ls.remove(symbol)
                print("error3", self.strike_sym_ls)
        ...
        for i in data:
            ...
            if i.get('state') == 'filled' and i['side'] == 'buy':  #  Purchase transaction
                if symbol in self.spot_profit_dict:
                    ...
                    continue
                elif symbol not in self.strike_sym_ls and symbol not in self.spot_ls:
                    self.strike_sym_ls.append(symbol)
