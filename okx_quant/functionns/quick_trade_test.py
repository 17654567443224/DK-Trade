...
from utils import deal_message, retry_on_exception_sync
...
class websocket_funs:
    def __init__(self, user):
        ...
        self.liqpx_slip = setting.liqpx_slip / 3
        self.Trigger_slip = setting.Trigger_slip / 3
        self.order_wait_time = setting.order_wait_time
        self.quick_tradingview = setting.quick_tradingview
        self.profit_per = setting.quick_profit_per
        self.stop_loss = setting.quick_stop_loss
        ...
        self.wx = wx_robot(user, "quick")
        self.wx_switch = False
        self.symbol_temp = []
        self.all_positions = []
        self.open_tag = "quickopen"
        self.close_tag = "quickclose"
            ...
            try:
                ...
                for i in result:
                    if i['maxSz'] is None or i['minSz'] is None:
                        ...
                        if Decimal(size) <= Decimal(i['maxSz']):
                            return size
    ...
    def dynamic_param(self, symbol):
        ...
        sum_price = abs(sum_price) / 2  # 2
        sum_price = Decimal(sum_price)
        # Map the results to the weight range through an exponential function
        self.liqpx_slip = Decimal(setting.liqpx_slip) * sum_price
        self.order_wait_time = setting.order_wait_time - 12 * sum_price
        self.profit_per = Decimal(self.profit_per) + Decimal('0.01') * sum_price
        self.stop_loss = Decimal(self.stop_loss) - Decimal('0.01') * sum_price
        self.position_update = setting.position_update - 10 * sum_price
        self.profit_min = Decimal(setting.profit_min) + Decimal('0.05') * sum_price
        self.float_val = Decimal(setting.float_val) + Decimal('0.005') * sum_price
    ...
    def catch_error_sCode(self, code:str):
        if code == '51004':
            ...
            if setting.quick_circulate_switch:
                self.multiplier = 1
    ...
    def trade_logic(self, trading_view, bv, lever, symbol, price, ba, ctVal):  # trade_logic
        ...
        if pos_val <= setting.quick_extra:  # 2
            usdt = ba / Decimal(setting.total_fail_count)
        else:
            usdt = ba / Decimal(setting.total_fail_count) * Decimal(self.multiplier) / Decimal(setting.Incremental_multiplier) ** Decimal(setting.quick_extra)
        if usdt > bv:
            self.trade_switch = True
            if setting.quick_circulate_switch:
                self.multiplier = 1
                self.stop_loss = self.stop_loss * Decimal('3')
                self.profit_per = self.profit_per * Decimal('3')
                if self.stop_loss <= -0.5:
                    print("end,exit")
                    exit()
            print("account_empty", bv)
            return
        ...
        if fee_percent >= setting.quick_fee_percent:
            self.ignore_temp.append(symbol + '-SWAP')
            self.trade_switch = True
            return
        if trading_view == 1:  # 多  反向
            slPx = price * (Decimal('1') + Decimal(self.stop_loss) / Decimal(lever))
            sl_tg = slPx + price * Decimal(self.Trigger_slip)
            tpPx = price * (Decimal("1") + Decimal(self.profit_per) / Decimal(lever))  # 50 = 0.02 = 1.02
            tp_tg = tpPx - price * Decimal(self.Trigger_slip)
            slPx = self.match_precision(slPx, price)
            sl_tg = self.match_precision(sl_tg, price)
            tpPx = self.match_precision(tpPx, price)
            tp_tg = self.match_precision(tp_tg, price)
            result = self.accountAPI.set_leverage(instId=f'{symbol}-SWAP', lever=lever,
                                                  mgnMode='isolated', posSide='long')  # set lever
            res = self.tradeAPI.place_order(instId=f'{symbol}-SWAP', tdMode='isolated', side='buy', posSide='long', px="", tag=self.open_tag,
                                               ordType='market', sz=sz, banAmend='',
                                               quickMgnType='manual', tpTriggerPx=tp_tg, tpOrdPx=tpPx, slTriggerPx=sl_tg, slOrdPx=slPx, tpTriggerPxType='mark', slTriggerPxType='mark')
            ...
            slPx = price * (Decimal('1') - Decimal(self.stop_loss) / Decimal(lever))
            sl_tg = slPx - Decimal(price) * Decimal(self.Trigger_slip)
            tpPx = price * (Decimal("1") - Decimal(self.profit_per) / Decimal(lever))  # 50 = 0.02 = 1.02
            tp_tg = tpPx + Decimal(price) * Decimal(self.Trigger_slip)
            slPx = self.match_precision(slPx, price)
            sl_tg = self.match_precision(sl_tg, price)
            tpPx = self.match_precision(tpPx, price)
            tp_tg = self.match_precision(tp_tg, price)
            ...
            res = self.tradeAPI.place_order(instId=f'{symbol}-SWAP', tdMode='isolated', side='sell',tag= self.open_tag,
                                            posSide='short',
                                            ordType='market', sz=sz, banAmend='', px="",
                                            quickMgnType='manual', tpTriggerPx=tp_tg, tpOrdPx= tpPx, slTriggerPx=sl_tg, slOrdPx=slPx, tpTriggerPxType='mark', slTriggerPxType='mark')
            order_id = deal_message(res)[0].get('ordId')
            code = res.get('code')
            if code != '0':
    ...
    def trading(self, tradingview, symbol, price):  # 1.get_banlance  2.get_max_lever and position_cont(usdt)
        ...
        bv = bv * Decimal(setting.quick_proportion)
        ba = ba * Decimal(setting.quick_proportion)
        if int(ba) < 10:
            exit()
        result = deal_message(self.publicAPI.get_instruments(instType='SWAP', instFamily=symbol))
        lever = setting.quick_lever_max
    ...
    def run_position(self, result):
        for i in result:
            ...
            if symbol in self.strike_sym_ls:  # self.strike_sym_ls????????
                if Decimal(i['uplRatio']) > Decimal(self.profit_per) * Decimal('1.25') or Decimal(i['uplRatio']) < Decimal(self.stop_loss) * Decimal('1.05'):
    ...
    def deal_order(self, data):
        for i in data:
            ...
            if i.get('state') == 'filled' and i['side'] == 'buy':  #  Purchase transaction
                if tag == self.open_tag and source != '7':
                ...
                elif self.trade_switch == False and source == '7' and symbol in self.strike_sym_ls:
                    self.strike_sym_ls = []
                    ...
                    self.ignore_temp = []
                    self.trade_switch = True
                    self.profit_per = Decimal(setting.quick_profit_per)
                    self.stop_loss = Decimal(setting.quick_stop_loss)
                ...
                elif ordType == 'market' and symbol in self.strike_sym_ls:
                    if Decimal(i['pnl']) > 0:
                        self.multiplier = 1
                        try:
                            self.wx.push_order(symbol, "市价止盈", self.multiplier, i['pnl'])
                        except Exception as e:
                            print(e)
                    else:
                        self.multiplier = self.multiplier * setting.Incremental_multiplier
                        try:
                            self.wx.push_order(symbol, "市价止损", self.multiplier, i['pnl'])
                        except Exception as e:
                            print(e)
                    self.profit_per = Decimal(self.profit_per) + Decimal('0.01')
                    self.stop_loss = Decimal(self.stop_loss) - Decimal('0.01')
                    self.ignore_temp = []
                    self.strike_sym_ls = []
                    self.trade_switch = True
            ...
            print(self.trade_switch, self.all_positions, self.strike_sym_ls)
    ...
    def run(self):
        if not setting.quick_trading_switch:
            return
        self.wx.start()
