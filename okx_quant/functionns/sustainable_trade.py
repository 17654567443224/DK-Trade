...
class websocket_funs:
    def __init__(self, user):
        ...
        self.profit_per = setting.sustainable_profit_per
        self.stop_loss = setting.sustainable_stop_loss
        ...
        self.wx = wx_robot(user, "sustainable")
        self.wx_switch = False
        self.symbol_temp = []
        self.all_positions = []
        self.open_tag = "susopen"
        self.close_tag = "susclose"
        ...
        if result:
            for i in result:
                if i.get('state') == 'live':
                    ...
                    if e_time > self.order_wait_time and sym in self.order_dict and i['algoId'] == '' and tag == self.open_tag:
        ...
        if symbol in result:
            if symbol not in self.strike_sym_ls and symbol not in self.order_dict:
                if Decimal(data[0][2]) > Decimal(result[symbol][0]) and Decimal(data[0][3]) > Decimal(result[symbol][1]):  # 0：timestamp 1：o 2：h 3：l 4：c
                    ...
                    if (Decimal(new_res[0][2]) - Decimal(new_res[1][2])) / (Decimal(new_res[0][2]) - Decimal(new_res[0][3])) > 0.382 or self.target_switch == True:
                    ...
                    if max(target_temp) < 0:
                        self.target_switch = True
                    if (Decimal(new_res[1][3]) - Decimal(new_res[0][3])) / (Decimal(new_res[0][2]) - Decimal(new_res[0][3])) > 0.382 or self.target_switch == True:
                        trading_view = 0  # short
                        print("Trigger_short", symbol)
                        price = Decimal(data[0][3]) * Decimal(1.0005)  # 1.0005
                        price = self.match_precision(Decimal(price), Decimal(data[0][3]))
    ...
    def dynamic_param(self, symbol):
        ...
        sum_price = abs(sum_price)  # 2
        # Map the results to the weight range through an exponential function
        weight = 1 / math.exp(-sum_price)  # exp 7
        weight_d = 1 / weight  # 0.13
        sum_val = Decimal(str(1 - weight_d)).log10()  # -0.06
        sum_val = abs(math.floor(1/sum_val))  # ceil abs  17
        sum_val = sum_val * 0.7
        weight = weight * 0.7
        self.liqpx_slip = setting.liqpx_slip * weight
        self.order_wait_time = setting.order_wait_time / sum_val
        self.profit_per = self.profit_per * (sum_val / 100 + 1)
        self.stop_loss = self.stop_loss * (sum_val / 100 + 1)
    ...
    def catch_error_sCode(self, code:str):
        if code == '51004':
            ...
            if setting.sustainable_trade_switch:
                self.multiplier = 1
    ...
    @retry_on_exception_sync
    def trade_logic(self, trading_view, bv, lever, symbol, price, ba, ctVal):  # trade_logic
        self.dynamic_param(symbol)
        pos_val = math.log(self.multiplier, setting.Incremental_multiplier)  # 27 3**3
        if pos_val <= setting.sustainable_extra:  # 2
            usdt = bv / Decimal(setting.total_fail_count)
        else:
            usdt = bv / Decimal(setting.total_fail_count) * Decimal(self.multiplier) / Decimal(setting.Incremental_multiplier) ** Decimal(setting.sustainable_extra)
        if usdt > ba * Decimal(setting.sustainable_proportion):
            if setting.sustainable_trade_switch:
                self.multiplier = 1
            self.trade_switch = True
        ...
        if trading_view == 1:  # 多  反向
            result = self.accountAPI.set_leverage(instId=f'{symbol}-SWAP', lever=lever,
                                                  mgnMode='isolated', posSide='long')  # set lever
            res = self.tradeAPI.place_order(instId=f'{symbol}-SWAP', tdMode='isolated', side='buy', posSide='long', px=str(price), tag=self.open_tag,
    ...
    def trading(self, tradingview, symbol, price):  # 1.get_banlance  2.get_max_lever and position_cont(usdt)
        ...
        if not setting.steady_switch:
            bv = bv * Decimal(setting.sustainable_proportion)
        if int(ba) < 10:
            exit()
        ...
        if setting.sustainable_lever_min is not None:
            if int(lever) < setting.sustainable_lever_min and self.multiplier != setting.max_fail_count:
                self.ignore_temp.append(symbol+'-SWAP')
                self.trade_switch = True
                return
        self.trade_logic(tradingview, bv, lever, symbol, price, ba, ctVal)
    ...
    def run_position(self, result):
        for i in result:
            ...
            if symbol not in self.order_algo and symbol in self.strike_sym_ls:  # self.strike_sym_ls????????
                count = self.multiplier / setting.max_fail_count
                profit_per_temp = self.profit_per
                stop_loss_temp = self.stop_loss
                if count > 1:
                    cal = math.log(count, setting.Incremental_multiplier)
                    if cal >= setting.sustainable_extra_fail:
                        self.multiplier = 1
                    self.profit_per = Decimal(self.profit_per) + Decimal('0.02') * Decimal(cal)
                    self.stop_loss = Decimal(self.stop_loss) - Decimal('0.02') * Decimal(cal)
                lv = Decimal(setting.sustainable_lever_min) / Decimal(lever)
                if 1 < lv <= 20:
                    self.profit_per = Decimal(self.profit_per) - Decimal('0.002') * lv
                    self.stop_loss = Decimal(self.stop_loss) - Decimal('0.002') * lv
                if 1 > lv >= 0.05:
                    lv = Decimal('1') / lv
                    self.profit_per = Decimal(self.profit_per) + Decimal('0.004') * lv
                    self.stop_loss = Decimal(self.stop_loss) - Decimal('0.004') * lv

                if mode == 'long':
                    side = 'sell'
                    posSide = 'long'
                    slPx = avgpx + (abs(Decimal(avgpx) - Decimal(liqpx)) * Decimal(self.stop_loss) + Decimal(self.liqpx_slip)*avgpx)  # 11+
                    ...
                    side = 'buy'
                    posSide = 'short'
                    slPx = avgpx - (abs(Decimal(avgpx) - Decimal(liqpx)) * Decimal(self.stop_loss) - Decimal(self.liqpx_slip)*avgpx)
                    sl_tg = slPx - Decimal(avgpx) * Decimal(self.Trigger_slip)
                ...
                slPx = self.match_precision(slPx, markPx)
                sl_tg = self.match_precision(sl_tg, markPx)
                tpPx = self.match_precision(tpPx, markPx)
                tp_tg = self.match_precision(tp_tg, markPx)
                result = self.tradeAPI.place_algo_order(symbol, 'isolated', side, ordType='oco',tag=self.close_tag,
                ...
                if profit_per_temp is not None and stop_loss_temp is not None:
                    self.profit_per = profit_per_temp
                    self.stop_loss = stop_loss_temp
                if res[0].get('sCode') == '0':
                    algoId = res[0].get('algoId')
                    self.order_algo[symbol] = algoId
                    print("algo_order_place_success", avgpx, markPx)
                    continue
                else:
                    print(res[0].get('sMsg'), symbol, slPx, sl_tg, tpPx, tp_tg, avgpx, markPx)
                    continue
            if Decimal(i['uplRatio']) > Decimal(self.profit_per) * Decimal('1.25') or Decimal(i['uplRatio']) < Decimal(self.stop_loss) * Decimal('1.15'):
                if symbol in self.strike_sym_ls:
                    self.close_position(symbol, mode, sz)

            if Decimal(i['uplRatio']) > Decimal('0.3') and self.multiplier == 1 and self.wx_switch == True and symbol in self.strike_sym_ls:
                            ...
                            with self.lock:
                                if isinstance(self.fluctuation_values, list) and self.trade_switch == True:
                                    if sym_idx == len(self.fluctuation_values) - 1:
                                        ...
                                        sym_idx = sym_idx + 1
                                        continue
                                    elif self.fluctuation_values[sym_idx] == symbol and symbol not in self.ignore_temp and symbol not in self.symbol_temp and symbol not in setting.Black_list and symbol not in self.all_positions:
    ...
    def deal_order(self, data):
        for i in data:
            ...
            if i.get('state') == 'filled' and i['side'] == 'buy':  #  Purchase transaction
                if tag == self.open_tag:
                    ...
                    if self.profit_switch == False and Decimal(i['pnl']) > 0:  # Stop profit transactions
                        ...
                        self.profit_switch = False
                        self.profit_min = self.profit_min + Decimal('0.01')
                        self.position_time = 0
                        try:
            ...
            print(self.trade_switch, self.all_positions)
    ...
    def run(self):
        if not setting.sustainable_trade_switch:
            return
        self.wx.start()
